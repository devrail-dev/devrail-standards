...
2. **Critical rules inlined directly** -- the six non-negotiable rules that agents must follow regardless of whether they resolve cross-file references.
...
- **Inlined critical rules** ensure that even agents which ignore cross-references still follow the six most important behaviors. These rules represent the minimum viable compliance for any agent operating on a DevRail-managed project.
...
Every shim file inlines these six rules verbatim. They are extracted from the `<!-- devrail:critical-rules -->` section of DEVELOPMENT.md.
