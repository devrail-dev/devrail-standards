...
2. **Critical rules inlined directly** -- the seven non-negotiable rules that agents must follow regardless of whether they resolve cross-file references.
...
- **Inlined critical rules** ensure that even agents which ignore cross-references still follow the seven most important behaviors. These rules represent the minimum viable compliance for any agent operating on a DevRail-managed project.
...
Every shim file inlines these seven rules verbatim. They are extracted from the `<!-- devrail:critical-rules -->` section of DEVELOPMENT.md.
...
3. **Critical Rules** -- the six rules inlined verbatim.
