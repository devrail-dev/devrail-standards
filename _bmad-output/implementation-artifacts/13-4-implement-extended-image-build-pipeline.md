...
- [x] **Task 1: Dockerfile.devrail generator script** (AC: 1, 7)
  - [x] 1.1 Create `scripts/plugin-build-extended-image.sh`. Sources `lib/log.sh`, `lib/plugin-cache.sh`. Header in standard format.
  - [x] 1.2 Implement `generate_dockerfile <devrail.yml> <output-path>`:
  ...
  - [x] 1.3 Always start with `# Auto-generated by plugin-build-extended-image.sh; DO NOT EDIT.` header and `FROM ghcr.io/devrail-dev/dev-toolchain:v$(get_devrail_version) AS runtime`. Use a pinned tag, not `:v1` floating, so a v1 minor bump doesn't silently invalidate every consumer's cache.
  - [x] 1.4 Plugin order = lockfile order (alphabetical by source URL — already deterministic from Story 13.3) so dockerfile content hashes are stable.
  - [x] 1.5 Output exits 0 on success. Skip silently if no plugins declared (no Dockerfile.devrail written).

- [x] **Task 2: Host-side plugin cache mount** (AC: 6)
  - [x] 2.1 Add `DEVRAIL_HOST_PLUGINS_CACHE` Makefile variable defaulting to `${HOME}/.cache/devrail/plugins`.
  - [x] 2.2 Update `DOCKER_RUN` to bind-mount `$(DEVRAIL_HOST_PLUGINS_CACHE):/opt/devrail/plugins`. Always present (even with no plugins declared — the directory is just empty in that case).
  - [x] 2.3 Create the cache directory on the host before any docker invocation. Pattern: `mkdir -p $(DEVRAIL_HOST_PLUGINS_CACHE)` as a `_check-config` prereq, or as part of a new `_ensure-host-cache` target chained from `_check-config`.
  - [x] 2.4 Smoke-test: after `make plugins-update`, the host cache directory contains the expected `<slug>/<rev>/plugin.devrail.yml`. After `docker run --rm` finishes, the cache survives.

- [x] **Task 3: Build pipeline target — `_extended-image` and `_extended-image-resolve`** (AC: 2, 3)
  - [x] 3.1 Add `_extended-image` internal target that generates `Dockerfile.devrail` and builds the image. Wraps `docker build --quiet -t devrail-local:<hash> -f Dockerfile.devrail .` with structured event emission.
  - [x] 3.2 Hash the generated `Dockerfile.devrail` (sha256, first 16 hex chars) for the tag suffix.
  - [x] 3.3 Cache hit: if `docker image inspect devrail-local:<hash>` succeeds, skip the build, emit `extended image cache hit`.
  - [x] 3.4 Build failures: capture stderr, emit `error`-level event with `failed_plugin` (last plugin section that started in the dockerfile) and `stderr_tail` (last 20 lines).
  - [x] 3.5 On success, write the chosen tag to `.devrail/extended-image-tag` (workspace-local, gitignored) so other targets can read it without re-hashing.
  - [x] 3.6 Cache-hit timing target: < 1s wall (verified by smoke test). Include `duration_ms` in the cache-hit event.

- [x] **Task 4: DOCKER_RUN swap-in for plugin-using projects** (AC: 4, 5)
  - [x] 4.1 Add `DEVRAIL_RESOLVED_IMAGE` Make-time variable: defaults to `$(DEVRAIL_IMAGE):$(DEVRAIL_TAG)`; if `.devrail/extended-image-tag` exists and `HAS_PLUGINS` is non-empty, override to that tag.
  - [x] 4.2 Update `DOCKER_RUN` to use `$(DEVRAIL_RESOLVED_IMAGE)` instead of `$(DEVRAIL_IMAGE):$(DEVRAIL_TAG)`.
  - [x] 4.3 Add `_extended-image` to the prereq chain of public host targets (`check`, `lint`, etc.) so the image is built before `DOCKER_RUN` is invoked. Tricky because `DOCKER_RUN` is `:=` immediate-eval — solution: split into two-stage host-target. First stage materializes the image and writes the tag file; second stage reads the tag file and runs the in-container target. Pattern: `check: _extended-image && $(DOCKER_RUN_DYNAMIC) make _check` where `DOCKER_RUN_DYNAMIC` is recursively-expanded.
  - [x] 4.4 No-plugins regression: when `HAS_PLUGINS` is empty, `DEVRAIL_RESOLVED_IMAGE` stays at the core image; no Dockerfile.devrail written; no docker build invoked.
  - [x] 4.5 Add `.devrail/` to a project-template `.gitignore` snippet (touched in Story 13.6 as part of the v1.11.0 release prep).

- [x] **Task 5: Test fixtures + smoke test** (AC: 8)
  - [x] 5.1 Create `tests/fixtures/plugin-repos/extended-elixir-v1/`:
  ...
  - [x] 5.2 Create `tests/test-plugin-build-pipeline.sh`. Pattern mirrors `tests/test-plugin-resolver.sh` (mktemp + git-init harness + `run_make` helper).
  - [x] 5.3 **Case 1: dockerfile generation determinism** — write a `.devrail.yml` with one plugin, run `_extended-image` twice, sha256 the generated `Dockerfile.devrail` both times, assert equal.
  - [x] 5.4 **Case 2: build cache hit** — run `_extended-image` twice, assert second invocation emits `extended image cache hit` (not `extended image built`); assert wall time < 1s for second.
  - [x] 5.5 **Case 3: no plugins → no Dockerfile.devrail, no devrail-local image** — `.devrail.yml` with `languages: [bash]`, no `plugins:`. Run `_extended-image`. Assert `Dockerfile.devrail` does NOT exist; assert `docker images -q devrail-local` is empty (or unchanged).
  - [x] 5.6 **Case 4: install_script executes** — build, then `docker run --rm devrail-local:<hash> bash -c 'cat /opt/devrail/plugins/<slug>/install.sh'` to verify the script is in the image; or `docker run --rm devrail-local:<hash> printenv TEST_PLUGIN_VAR` to verify the env was applied.
  - [x] 5.7 **Case 5: multi-plugin layering** — declare two plugins in `.devrail.yml`, build, assert the resulting image has both plugins' env vars present.
  - [x] 5.8 **Case 6: build failure surfaces structured event** — fixture plugin with `install_script` that exits 1. Run `_extended-image`. Assert exit 2; assert `error` event with `failed_plugin` and `stderr_tail`.
  - [x] 5.9 **Case 7: host cache persistence** — run `make plugins-update`, then check `${HOME}/.cache/devrail/plugins/<slug>/<rev>/plugin.devrail.yml` exists. Run `make check` (which would trigger another container invocation), confirm the manifest is still there.
  - [x] 5.10 Use `read_via_docker` helper (sidecar container with `:ro` mount) for cache-content checks since the cache is root-owned 0700 inside docker.

- [x] **Task 6: Wire smoke test into CI** (AC: 8)
  - [x] 6.1 Add a step in `.github/workflows/ci.yml` after the existing plugin resolver smoke test: `bash tests/test-plugin-build-pipeline.sh`.

- [x] **Task 7: Documentation + CHANGELOG**
  - [x] 7.1 CHANGELOG.md `[Unreleased]` → `### Added`: extended-image build pipeline, host-side persistent plugin cache.
  - [x] 7.2 Update STABILITY.md "Plugin loader + resolver + lockfile" row to "Plugin loader + resolver + lockfile + build pipeline" (still Preview; execution loop in Story 13.5).
  - [x] 7.3 Document the `${HOME}/.cache/devrail/plugins` host directory in STABILITY.md "Consumer responsibilities" section (existing area for postgres/bundle install notes).
  - [x] 7.4 No changes yet to `standards/devrail-yml-schema.md` — Story 13.6 bundles all schema/standards-doc updates.
...
Claude Opus 4.7 (1M context) — split into two PRs in a single session at user direction.

### Debug Log References

- **Story split into 13.4a + 13.4b at user direction**: 13.4a = host cache mount + Dockerfile.devrail generator + Story 13.3 chmod fold-in (PR #36, merged). 13.4b = `_extended-image` build target + DOCKER_RUN swap-in + full build smoke (PR #37). Each piece independently tested and shippable; together they implement the original Story 13.4 AC.
- **DOCKER_RUN expansion change**: was `:=` (immediate), now `=` (recursive) — needed so `DEVRAIL_RESOLVED_IMAGE` re-reads `.devrail/extended-image-tag` each invocation, after `_extended-image` has produced it. Without this, immediate evaluation captures the tag at make-time before the prereq has run.
- **Test fixture iteration**: initial fixture (`extended-elixir-v1`) used `debian:bookworm-slim` + `apt_packages: [jq]` to exercise the full apt path, but the build was too slow for hermetic testing. Switched to `minimal-v1` fixture with no apt packages and no copy_from_builder paths — uses the dev-toolchain image itself as the base_image so the build is essentially `FROM + ENV + COPY install.sh + RUN install.sh`. Sub-second build per test invocation.
- **Hermetic test strategy**: `make plugins-update` requires a network-reachable git source. To keep smoke tests fully hermetic (no git, no network), Cases 5-8 pre-populate the host cache directly via `cp` into a docker container and hand-craft `.devrail.lock` with the correct content_hash. This isolates the build pipeline as the system under test rather than coupling it to the resolver.
- **Install script marker path**: the fixture's install.sh originally hardcoded `/opt/devrail/plugins/minimal/.install-marker` but the test used a custom slug (`test-plugin-minimal`), so the path didn't exist. Fixed by having install.sh write the marker to its own directory (`$(dirname "$0")`).
- **shellcheck SC2317 on cleanup_staging**: trap-invoked function flagged as "unreachable". Added `# shellcheck disable=SC2317` directive (existing pattern from `lib/log.sh`).
- **`make _check` host-side debugging gotcha**: ran `make _check` directly on host (not via DOCKER_RUN) which failed because `/opt/devrail/scripts/` doesn't exist on host. Use `make check` (public) for end-to-end self-checks.
...
**Implementation summary (2026-05-03/04):**

- All 8 AC satisfied across 2 PRs (story split at user direction for risk reduction).
- 13.4a (PR #36, merged → on main): host cache mount, generator script, chmod fold-in, 4-case generator unit smoke.
- 13.4b (PR #37, in flight): `_extended-image` build target, `DEVRAIL_RESOLVED_IMAGE` swap-in, 5 additional smoke cases (real build, cache hit, no-plugins regression, install_script execution, build failure).
- Total tests: 9-case smoke covering generator unit + full build pipeline. All regression-safe (resolver 16/16, loader 11/11, Rails 4/4, self-check pass).
- `make check` on dev-toolchain itself is regression-safe — no plugins declared, build pipeline is no-op, loader emits "no plugins declared" info event.
- Host orchestrator pattern: bash on the host, calls into the container for `_generate-dockerfile` (in-container, depends on `_plugins-load` for cache population), then runs `docker build` host-side. Cache-hit path uses `docker image inspect` (instant).
- DOCKER_RUN swap-in via recursively-expanded variable: clean integration with the existing two-layer delegation pattern.

**Verification (all green locally against the freshly built image):**

- `tests/test-plugin-build-pipeline.sh` — 9/9
- `tests/test-plugin-resolver.sh` — 16/16
- `tests/test-plugin-loader.sh` — 11/11
- `tests/smoke-rails.sh` — 4/4
- `make check` on dev-toolchain itself — pass

**Implementation PRs:**
- 13.4a: https://github.com/devrail-dev/dev-toolchain/pull/36 (merged)
- 13.4b: https://github.com/devrail-dev/dev-toolchain/pull/37 (in review)

### File List

**Implementation 13.4a (dev-toolchain repo, PR #36, merged):**
- `scripts/plugin-build-extended-image.sh` — new (Dockerfile.devrail generator)
- `lib/plugin-cache.sh` — pre-existing from Story 13.3 review (used by generator and verifier)
- `Makefile` — modified (DEVRAIL_HOST_PLUGINS_CACHE variable, _ensure-host-cache target, host cache bind-mount in DOCKER_RUN, prereq wires)
- `scripts/plugin-resolver.sh` — modified (chmod 0755 fold-in for host-readability of cached tree)
- `tests/test-plugin-build-pipeline.sh` — new (4 generator unit cases)
- `.github/workflows/ci.yml` — modified (Plugin build-pipeline smoke test step)
- `CHANGELOG.md`, `STABILITY.md` — modified

**Implementation 13.4b (dev-toolchain repo, PR #37, branch `feat/13-4b-build-pipeline`):**
- `scripts/plugin-extended-image.sh` — new (HOST orchestrator: stages build context, runs generator, hashes, builds, writes tag)
- `Makefile` — modified (DEVRAIL_RESOLVED_IMAGE recursively-expanded var, DOCKER_RUN := → =, _generate-dockerfile + _extended-image targets, prereq wires on public targets)
- `tests/fixtures/plugin-repos/minimal-v1/` — new hermetic fixture (plugin.devrail.yml, install.sh, README.md)
- `tests/test-plugin-build-pipeline.sh` — extended (5 full-pipeline cases added; total 9)
- `CHANGELOG.md`, `STABILITY.md` — modified

**Story tracking (OrgDocs repo, branch `feat/13-4-create-story`):**
- `_bmad-output/implementation-artifacts/13-4-implement-extended-image-build-pipeline.md` — modified (status, all 30+ task checkboxes, Dev Agent Record, File List)
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — modified (`13-4`: `ready-for-dev` → `in-progress` → `review`)
